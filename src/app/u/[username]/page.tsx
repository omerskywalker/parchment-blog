...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    ...
    openGraph: {
      title: `${displayName} on Parchment`,
      description: user.bio ?? `Read posts by ${displayName} on Parchment.`,
      type: "profile",
      url: `/u/${user.username}`,
    },
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                ...
                {/* RSS link for this author's posts — useful for readers */}
                <Link
                  href={`/rss.xml`}
              ...
              {user.bio ? <p className="mt-3 text-sm leading-relaxed text-white/70">{user.bio}</p> : null}
