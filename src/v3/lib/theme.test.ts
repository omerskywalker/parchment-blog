...
// We're operating on a minimal subset of `window` and `document` — just
// enough surface for theme.ts. Module-level handles let the tests avoid
// fighting the global Window/Document types from lib.dom.d.ts.
...
describe("applyTheme", () => {
  it("sets the data-theme attribute on <html>", () => {
    applyTheme("sepia");
    expect(doc.documentElement.getAttribute(THEME_DOM_ATTRIBUTE)).toBe(
      "sepia",
    );
  });
...
describe("getThemeBootScript — FOUC prevention contract", () => {
  it("includes the storage key, dom attribute, and both theme values", () => {
    // The script must reference the same constants as the rest of the system.
    // This test catches accidental rename drift.
